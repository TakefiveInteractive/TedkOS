...
chunk_info_t wav_read_chunk_header() {
    ...
    info.chunk_id = 0;
    info.data_size = 0;
    uint32_t buf[5] = {0}; // used to be uint8_t buf[13]
    // read in the chunk header
    if (theDispatcher->read(status.fd, (uint8_t*)buf, 8) < 8) {
        return info;
    }
    info.chunk_id = * (buf + 0);
    info.data_size = * (buf + 1);
    if (info.chunk_id == RIFF) {
        theDispatcher->read(status.fd, (uint8_t*)(buf + 2), 4);
        if (*(buf + 2) != WAVE) {
...
uint16_t get_int16(void *buf, int offset) {
    return *(uint16_t*) ((uint32_t)buf + offset);
}

uint32_t get_int32(void *buf, int offset) {
    return *(uint32_t*) ((uint32_t)buf + offset);
}
...
int32_t play_wav(char *filename) {
    ...
    uint32_t bytes_read = theDispatcher->read(status.fd, format, info.data_size);
