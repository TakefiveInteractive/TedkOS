...
namespace Term
{
    ...
    int32_t Term::read(filesystem::File& fdEntity, uint8_t *buf, int32_t bytes)
    {
        ...
        if(term_buf_pos < nbytes)
        {
            ...
            lock.waitUntil([](){ return !UserWaitingRead; });
        }
    ...
    int32_t Term::write(filesystem::File& fdEntity, const uint8_t *buf, int32_t bytes)
    {
